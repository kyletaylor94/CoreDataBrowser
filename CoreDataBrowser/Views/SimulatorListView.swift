...
struct SimulatorListView: View {
    ...
    var body: some View {
        ...
        .onAppear(perform: simulatorViewModel.loadSimulators)
        .searchable(text: searchBinding)
        .alert("Error!", isPresented: errorBinding, presenting: simulatorViewModel.currentError) { error in
            Button("OK") { simulatorViewModel.shouldShowError = false }
        } message: { error in
